    ...
    public function handle()
    {
        ...
        if (count($event_reg) > 0) {
            foreach ($event_reg as $value) {
                if (isset($value->getEventProgram) && count($value->getEventProgram)) {
                    $check = 1;
                    foreach ($value->getEventProgram as $event_prog) {
                        $check_survey = 0;
                        if (isset($event_prog->getProgramDetails) && ($event_prog->getProgramDetails->survey == 'yes' && $event_prog->getProgramDetails->survey_activity_uuid != null)) {
                            $check_survey = 1;
                        }
                        if ($check_survey === 1) {
                ...
                if ($check == 0 && isset($value->getEventRegistrationOptional)) {
                    if (count($value->getEventRegistrationOptional) > 0) {
                        $check = 1;
                        foreach ($value->getEventRegistrationOptional as $event_opt_reg) {
                            $check_survey = 0;
                            if (isset($event_opt_reg->getOptionalDetails) && ($event_opt_reg->getOptionalDetails->survey == 'yes' && $event_opt_reg->getOptionalDetails->survey_activity_uuid != null)) {
                                $check_survey = 1;
                            }
                            if ($check_survey === 1) {
