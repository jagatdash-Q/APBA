    ...
    public function handle()
    {
        ...
        try {
            ...
            foreach ($votings as $index => $value) {
                // Get the positions
                if (count($value->getVotingPositions) > 0) {
                    ...
                    foreach ($value->getVotingPositions as $val) {
                        if ($val->getNominationPositionDetails != null && $val->getNominationPositionDetails->getPosition != null) {
                            $voting_positions[] = $val->getNominationPositionDetails->getPosition->membership_position;
                        }
                    }
            ...
            if (count($voting_details)) {
                foreach ($voting_details as $vot) {
                    ...
                    foreach ($customers as $val) {
                        ...
                        if ($count <= 125 && $val->getActiveSubscriptionDetails != null) {
        ...
        try {
            ...
            if (count($voting_details)) {
                foreach ($voting_details as $vot) {
                    ...
                    $v_count = 1;
                    foreach ($customers as $val) {
                        if ($v_count <= 125 && $val->getActiveSubscriptionDetails != null) {
                            // Sent mail on start day
                            if (strpos($val->getActiveSubscriptionDetails, "Corporate") == false && Carbon::parse($vot['voting_start_date'])->format('Y-m-d') === Carbon::now()->format('Y-m-d')) {
                                // Check if mail sent or not 
                    ...
                    $val_count = 1;
                    foreach ($customers as $val) {
                        if ($val_count <= 125 && $val->getActiveSubscriptionDetails != null) {
                            if (strpos($val->getActiveSubscriptionDetails, "Corporate") == false && Carbon::parse($vot['voting_start_date'])->format('Y-m-d') !== Carbon::now()->format('Y-m-d')) {
                                // date difference 
                                ...
                                if ($end_date->gt($current_date) && $days_difference == 30) {
                    ...
                    $var14_count = 1;
                    foreach ($customers as $val) {
                        if ($var14_count <= 125 && $val->getActiveSubscriptionDetails != null) {
                            if (strpos($val->getActiveSubscriptionDetails, "Corporate") == false && Carbon::parse($vot['voting_start_date'])->format('Y-m-d') !== Carbon::now()->format('Y-m-d')) {
                                // date difference 
                                $end_date = Carbon::parse(Carbon::parse($vot['voting_end_date'])->format('Y-m-d'));
                                $current_date = Carbon::parse(Carbon::now()->format('Y-m-d'));
                                $days_difference = $current_date->diffInDays($end_date);
                                if ($end_date->gt($current_date) && $days_difference == 14) {
                                    $is_mail_sent = VotingReminderMailSentStatus::where('customer_id', $val->id)->where('voting_uuid', $vot['uuid'])->where('mail_sent_for', $days_difference)->first();
                                    if ($is_mail_sent == null) {
                                        $details = [
                                            'name' => $val->user_name,
                    ...
                    $var7_count = 1;
                    foreach ($customers as $val) {
                        if ($var7_count <= 125 && $val->getActiveSubscriptionDetails != null) {
                            if (strpos($val->getActiveSubscriptionDetails, "Corporate") == false && Carbon::parse($vot['voting_start_date'])->format('Y-m-d') !== Carbon::now()->format('Y-m-d')) {
                                // date difference 
                                $end_date = Carbon::parse(Carbon::parse($vot['voting_end_date'])->format('Y-m-d'));
                                $current_date = Carbon::parse(Carbon::now()->format('Y-m-d'));
                                $days_difference = $current_date->diffInDays($end_date);
                                if ($end_date->gt($current_date) && $days_difference == 7) {
                                    $is_mail_sent = VotingReminderMailSentStatus::where('customer_id', $val->id)->where('voting_uuid', $vot['uuid'])->where('mail_sent_for', $days_difference)->first();
                                    if ($is_mail_sent == null) {
                                        $details = [
                                            'name' => $val->user_name,
                    ...
                    $var1_count = 1;
                    foreach ($customers as $val) {
                        if ($var1_count <= 125 && $val->getActiveSubscriptionDetails != null) {
                            if (strpos($val->getActiveSubscriptionDetails, "Corporate") == false && Carbon::parse($vot['voting_start_date'])->format('Y-m-d') !== Carbon::now()->format('Y-m-d')) {
                                // date difference 
                                $end_date = Carbon::parse(Carbon::parse($vot['voting_end_date'])->format('Y-m-d'));
                                $current_date = Carbon::parse(Carbon::now()->format('Y-m-d'));
                                $days_difference = $current_date->diffInDays($end_date);
                                if ($end_date->gt($current_date) && $days_difference == 1) {
                                    $is_mail_sent = VotingReminderMailSentStatus::where('customer_id', $val->id)->where('voting_uuid', $vot['uuid'])->where('mail_sent_for', $days_difference)->first();
                                    if ($is_mail_sent == null) {
                                        $details = [
                                            'name' => $val->user_name,
