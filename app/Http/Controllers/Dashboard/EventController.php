...
class EventController extends Controller
{
    ...
    public function manageEvents(Request $request)
    {
        ...
        return view('dashboard.event-manager.manage-events', compact('events'));
    }
            ...
            try {
                if ($is_event_create) {
                    ...
                    for ($sp_count = 1; $sp_count <= $request->featured_speakers_add_more_count; $sp_count++) {
                        ...
                        if ($request->has($featured_speaker_name)) {
                            if ($request->$featured_speaker_name != '' && $request->$featured_speaker_designation != '' && $request->$featured_speaker_url != '') {
                                    ...
                                    if ($is_event_tab_create) {
                                        ...
                                        for ($j = 1; $j <= $request->$total_add_more_button; $j++) {
                                            ...
                                            if ($request->has($btn_title)) {
                                                if ($request->$btn_title != '' && $request->$btn_label != '' && $request->$btn_link != '') {
    ...
    public function editEvent(Request $request, $slug)
    {
        ...
        if ($events == null) {
            ...
            return view('dashboard.event-manager.edit-event', compact('events'));
        }
    }
    ...
    public function eventDashboard(Request $request)
    {
        ...
        return view('dashboard.event-manager.dashboard', compact('reg_summery', 'years', 'month', 'all_events', 'user_workshop'));
    }
    public function eventDetails($uuid, $type = null, $survey_uuid = null)
    {
        ...
        if ($event_reg_details == null)
            abort(404);
        return view('dashboard.event-manager.event-workshop-details', compact('event_reg_details', 'survey'));
    }
