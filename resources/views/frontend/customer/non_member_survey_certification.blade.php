    ...
    <div class="survey_home">
        <div class="survey_page_container" id="welcome_container">
            ...
            <input type="hidden" name="survey_activity_uuid" id="survey_activity_uuid"
                value="{{ $survey_activity->uuid }}">
            <h1 class="survey_heading_h1 mb-3">
                {{ isset($survey_activity->getEventProgram->getEventDetails->event_name) ? $survey_activity->getEventProgram->getEventDetails->event_name : $survey_activity->getEventOptional->getEvent->event_name }}
            </h1>
            <h3 class="survey_heading_h3">
                @if (isset($survey_activity->getEventProgram))
                    {{ isset($survey_activity->getEventProgram->getEventWorkshopDetails->workshop_name) ? $survey_activity->getEventProgram->getEventWorkshopDetails->workshop_name : '' }}
                    |
                    {{ isset($survey_activity->getEventProgram->program_name) ? $survey_activity->getEventProgram->program_name : '' }}
                    |
                    {{ isset($survey_activity->getEventProgram->program_date) ? Carbon\Carbon::parse($survey_activity->getEventProgram->program_date)->format('d F Y') : '' }}
                @else
                Conference
                @endif
                                ...
                                <td>
                                    <div class="survey_radio_container">
                                        <input class="survey_radio_input"
                                            ...
                                            type="radio" name="content_material" id="content_material" value="9">
                ...
                switch (action) {
                    ...
                    case 'next':
                        ...
                        if (divID == 7) {
                            ...
                            $.ajax({
                                url: "{{ route('non.member.submit.survey') }}",
                                type: "POST",
                                data: {
                                    '_token': "{{ csrf_token() }}",
