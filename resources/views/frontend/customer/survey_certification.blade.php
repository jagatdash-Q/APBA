...
<body>
    <div class="survey_home">
        <div class="survey_page_container" id="welcome_container">
            ...
            @endif
            <input type="hidden" name="event_program_registration_uuid" id="event_program_registration_uuid"
                value="{{ $event_program->uuid }}">
            <input type="hidden" name="survey_activity_uuid" id="survey_activity_uuid"
                value="{{ $event_program->getProgramDetails->survey_activity_uuid }}">
            <h1 class="survey_heading_h1 mb-3">
                {{ isset($event_program->getProgramDetails->getEventDetails->event_name) ? $event_program->getProgramDetails->getEventDetails->event_name : '' }}
            </h1>
            <h3 class="survey_heading_h3">
                {{ isset($event_program->getProgramDetails->getEventWorkshopDetails->workshop_name) ? $event_program->getProgramDetails->getEventWorkshopDetails->workshop_name : '' }}
                |
                ...
                switch (action) {
                    ...
                    case 'next':
                        ...
                        if (divID == 7) {
                            ...
                            $.ajax({
                                url: "{{ route('customer.submit.survey') }}",
                                type: "POST",
                                data: {
                                    '_token': "{{ csrf_token() }}",
                                    "event_program_registration_uuid": $('#event_program_registration_uuid')
                                        .val(),
