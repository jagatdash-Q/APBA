...
@section('content')
    ...
    <div class="page_container">
        ...
        <div>
            <form action="{{ route('customer.register') }}" method="POST" id="membership_form"
                onsubmit="return checkConfirmPass()" enctype="multipart/form-data">
                @csrf
                @if (count($packages) > 0)
                    <div class="table_div">
                                    ...
                                    <tr>
                                        <td style="padding-top: 20px">
                                            <div class="radio_container">
                                                <input class="radio_input" type="radio" name="package_id" id="package_id"
                                                    value="{{ $packages_data->id }}">
                                                <span class="radio_checkmark"></span>
                                                <h3 class="site_heading_h3 ml-3">{{ $packages_data->membership_name }}</h3>
                                            </div>
                                        ...
                                        <td style="padding-top: 20px">
                                            <h3 class="site_heading_h3">S$ {{ $packages_data->membership_price }}
                                                @php
                                                    if ($packages_data->membership_plan == 'yearly') {
                                                        echo '(Yearly)';
                                                    } else {
                                                        echo '(1-time fee)';
                                                    }
                ...
                {{-- <input type="hidden" name="amount" value="{{ $is_membership_exist->getPackageInfo->membership_price }}"> --}}
                <input type="hidden" name="payment_for" value="customer registration">

                <div class="row">
                    {{-- <input type="hidden" name="package" id="package"
                        value="@if ($is_membership_exist->getPackageInfo != null) {{ $is_membership_exist->getPackageInfo->id }} @endif" /> --}}
                    {{-- <input type="hidden" name="package_uid" id="package_uid"
                        value="@if ($is_membership_exist->getPackageInfo != null) {{ $is_membership_exist->getPackageInfo->uid }} @endif" /> --}}
                    ...
                    <div class="col-sm-6 mb-4">
                        ...
                        <input type="password" class="form-control custom_input" placeholder="Your password" name="password"
